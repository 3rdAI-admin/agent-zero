...
class MemoryConsolidator:
    ...
    async def process_new_memory(
        ...
        try:
            ...
            PrintStyle().warning(
                f"Memory consolidation timeout for area {area}, saving memory directly"
            )
            # Fallback: save the memory directly so it is not lost
            return await self._fallback_direct_save(
                new_memory, area, metadata, log_item
            )
    ...
    async def _fallback_direct_save(
        self,
        new_memory: str,
        area: str,
        metadata: Dict[str, Any],
        ...
        """Fallback: save the memory directly without consolidation so it is not lost."""
        try:
            db = await Memory.get(self.agent)
            if "timestamp" not in metadata:
                metadata["timestamp"] = self._get_timestamp()
            metadata["consolidation_action"] = "fallback_direct_save"
            memory_id = await db.insert_text(new_memory, metadata)
            if log_item:
                log_item.update(
                    result="Memory saved directly (consolidation fallback)",
                    memory_ids=[memory_id],
                    consolidation_action="fallback_direct_save",
                )
            return {"success": True, "memory_ids": [memory_id]}
        except Exception as save_err:
            PrintStyle().error(
                f"Fallback memory save also failed for area {area}: {str(save_err)}"
            )
            if log_item:
                log_item.update(result=f"Fallback memory save failed: {str(save_err)}")
...
def create_memory_consolidator(agent: Agent, **config_overrides) -> MemoryConsolidator:
    ...
    - processing_timeout_seconds: Timeout for consolidation processing (default 120)
    """
