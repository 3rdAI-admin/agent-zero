...
def wait_for_health(host: str, port: int, scheme: str = "http"):
    url = f"{scheme}://{host}:{port}/health"
    # Reason: when using self-signed TLS certs, we must skip certificate verification
    # for the health check to succeed against our own server.
    ctx = None
    if scheme == "https":
