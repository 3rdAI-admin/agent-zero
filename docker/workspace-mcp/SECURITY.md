...
- **Network:** In the Agent Zero stack the service only exposes port 8889 on the Docker network (no host port by default), so only containers on the same network can reach it.
...
See **[PRODUCTION.md](PRODUCTION.md)** for HTTPS (Caddy/nginx), firewall, and production checklist. See also [docs/setup/GOOGLE_WORKSPACE_MCP_CONTAINER.md](../../docs/setup/GOOGLE_WORKSPACE_MCP_CONTAINER.md).
