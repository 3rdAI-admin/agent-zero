...
- [x] Credentials directory restricted: `chmod 700 workspace-mcp-credentials` (run in repo root if you haven’t).
...
- Prefer not binding 8889 to `0.0.0.0` on the public internet; use a VPN or private network for remote clients.
...
If all clients run in the same Docker network (e.g. Agent Zero in the same stack), leave the service with only `expose: - "8889"` and no `ports:` so nothing is published on the host. Remote clients then reach Agent Zero (or another gateway) that proxies to `workspace_mcp:8889` internally.

---

Quick ref: [SECURITY.md](SECURITY.md) · [README.md](README.md) · [docs/setup/GOOGLE_WORKSPACE_MCP_CONTAINER.md](../../docs/setup/GOOGLE_WORKSPACE_MCP_CONTAINER.md)
