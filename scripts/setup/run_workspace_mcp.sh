...
# Agent Zero (in Docker) can then connect to it at http://host.docker.internal:8889/mcp
#
...
#
# Usage: ./scripts/setup/run_workspace_mcp.sh [port]
...
# Load .env from repo root so GOOGLE_OAUTH_CLIENT_ID and GOOGLE_OAUTH_CLIENT_SECRET are set
...
echo "Connect Agent Zero to: http://host.docker.internal:$PORT/mcp"
echo "First tool use will open a browser for OAuth on this host."
echo "If port $PORT is in use, run: $0 <port> (e.g. $0 8890)"
echo ""
